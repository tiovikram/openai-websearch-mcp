...
const ApproximateLocationSchema = z.object({
  ...
  approximate: z.object({
    country: z.string().length(2).optional().describe("Two-letter ISO country code (e.g., 'US')"),
    city: z.string().optional().describe("City name (e.g., 'San Francisco')"),
    region: z.string().optional().describe("Region or state (e.g., 'California')"),
    timezone: z.string().optional().describe("IANA timezone (e.g., 'America/Los_Angeles')")
  })
});

const WebSearchChatCompletionSchema = z.object({
  model: z.enum(["gpt-4o-search-preview", "gpt-4o-mini-search-preview"]).describe("Model to use for web search"),
  messages: z.array(
    z.object({
  ...
  web_search_options: z.object({
    user_location: ApproximateLocationSchema.optional().describe("Optional user location to refine search results"),
    search_context_size: z.enum(["low", "medium", "high"]).optional().describe("Amount of context retrieved from the web")
  }).optional().describe("Configuration options for web search")
});
...
async function performWebSearchChatCompletion(params: z.infer<typeof WebSearchChatCompletionSchema>) {
  try {
...
async function performWebSearchResponses(params: z.infer<typeof WebSearchResponsesSchema>) {
  try {
    ...
    tools: [
      {
        ...
        description: "Perform a web search using OpenAI's Chat Completions API with search-enabled models",
  ...
  try {
    ...
    switch (request.params.name) {
      case "web_search_chat_completion": {
        const args = WebSearchChatCompletionSchema.parse(request.params.arguments);
